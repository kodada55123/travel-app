...
def run_backtest(ticker, name, mkt_df):
    raw = yf.download(ticker, start=mkt_df.index[0].strftime('%Y-%m-%d'),
    ...
    df['StockRS']= df['Close'] / df['Close'].shift(RS_WINDOW)   # 個股 20日強度
    ...
    for i in range(len(df)):
        ...
        if not pos.is_empty():
            ...
            # 停利：≥20% 後跌破 10MA + 放量  ← 改為 10MA
            exit_profit = (ret >= PROFIT_LOCK_PCT and
                           vol_today > vol5 and close < ma10)
            ...
            # 急跌綠K（大盤大跌豁免）
            exit_drop = (not big_drop_mkt and
                         body_ret < -BODY_DROP_EXIT and vol_today > vol5)
        ...
        if row['Signal'] and capital >= MAX_CAP * 0.1:
            ...
            if sh > 0:
                capital -= sh * close * (1 + FEE_RATE)
                pos.add(sh, close, date)
                events.append({'Date': date, 'Price': close, 'Type': 'entry'})
...
def plot_results(ticker, name, df, trades_df, equity_df, events_df, metrics, start, end):
    ...
    yr = start[:4]
    fig.suptitle(f'{name}（{ticker.replace(".TW","")}）  v3 策略  {start} ~ {end}',
    ...
    if events_df is not None and not events_df.empty:
        style = {'entry':('lime','^',150,6), 'exit':('red','v',150,6),
                 'add_obv':('cyan','^',100,5), '停利10%':('gold','D',85,5), '停利40%':('darkorange','D',110,5)}
        for _, ev in events_df.iterrows():
            s = style.get(ev['Type'])
            if s: ax1.scatter(ev['Date'], ev['Price'], color=s[0], marker=s[1], s=s[2], zorder=s[3])
    handles, labels = ax1.get_legend_handles_labels()
    extra = [mpatches.Patch(color=c, label=l) for c,l in [
        ('lime','進場 ▲'),('red','出場 ▼'),('cyan','OBV加碼 ▲'),('gold','停利10% ◆'),('darkorange','停利40% ◆')]]
    ax1.legend(handles+extra, labels+[e.get_label() for e in extra], loc='upper left', fontsize=7.5, ncol=2)
    ax1.set_ylabel('股價 (TWD)'); ax1.set_title('股價走勢、均線與進出場點'); ax1.grid(alpha=0.3)
    ...
    ax2.legend(fontsize=8); ax2.set_ylabel('成交量'); ax2.set_title('成交量 vs 均量'); ax2.grid(alpha=0.3)
    ...
    ax3.legend(fontsize=8); ax3.set_ylabel('OBV'); ax3.set_title('OBV（能量潮）'); ax3.grid(alpha=0.3)
    ...
    ax4.legend(fontsize=8); ax4.set_ylabel('RS差值')
    ax4.set_title(f'個股 vs 大盤相對強弱（{RS_WINDOW}日漲幅差）'); ax4.grid(alpha=0.3)
    ...
    save_path = f'/Users/kochechin/Desktop/google/backtest/{ticker.replace(".TW","")}_v3b_{yr}.png'
...
def main():
    ...
    for (start, end) in PERIODS:
        ...
        for ticker, name in TICKERS.items():
            ...
            sub_mkt = mkt[(mkt.index >= start) & (mkt.index <= end)]
            result  = run_backtest(ticker, name, sub_mkt)
            if result[0] is None:
                print("  [警告] 資料不足，跳過。")
                continue
        ...
        grand_summary[yr] = period_metrics

    # 三年橫向對比
    print(f"\n{'#'*68}")
    print("  三年度橫向績效對比（總報酬率）")
    print(f"{'#'*68}")
    rows = {}
    for yr, pm in grand_summary.items():
        for name, m in pm.items():
            if name not in rows: rows[name] = {}
            rows[name][yr] = m.get('總報酬率 (%)', 'N/A')
    df_compare = pd.DataFrame(rows).T
    df_compare.columns = [f'{c}年報酬率(%)' for c in df_compare.columns]
